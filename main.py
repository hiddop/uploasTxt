    ...
    for i in range(start_idx, len(links)):
        ...
        try:
            if url.endswith(".pdf"):
                ...
                if thumb:
                    await bot.send_document(m.chat.id, document=f'{name.replace(".mp4", ".pdf")}', thumb=thumb, caption=f"Downloaded PDF: {name.replace('.mp4', '.pdf')}")
                else:
                    await bot.send_document(m.chat.id, document=f'{name.replace(".mp4", ".pdf")}', caption=f"Downloaded PDF: {name.replace('.mp4', '.pdf')}")

            else:
                os.system(cmd)
                if thumb:
                    await bot.send_video(m.chat.id, video=name, thumb=thumb, caption=f"Downloaded Video: {name}")
                else:
                    await bot.send_video(m.chat.id, video=name, caption=f"Downloaded Video: {name}")
